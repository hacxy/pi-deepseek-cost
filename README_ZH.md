...
> 一个 [Pi](https://pi.dev) 扩展：追踪你的 **DeepSeek 用量与费用** —— 状态栏实时显示会话累计费用，`/ds-cost` 浮动弹窗，中英双语，支持人民币/美元/欧元显示，官方峰谷计费。
...
- **`/ds-cost`**：浮动弹窗 —— 分模型 token 用量（缓存命中/未命中输入、输出）、缓存命中率、CNY/USD/EUR 费用明细、峰谷拆分、另一**官方**货币对照
...
- **中英双语 + 三币种**：语言与货币分离 —— `locale` 控制界面文字（默认 zh → ¥、en → $），`currency` 控制显示货币（`cny`/`usd` 为官方价；`eur` 按你配置的 `eurRate` 由官方美元价换算，显示带 `≈`）。弹窗内按 `L` 或全局快捷键 `Ctrl+Shift+L` 循环切换（¥ → $ → €）
...
| 费用详情弹窗          | `/ds-cost`（Esc 关闭，`L` 切换货币）                            |
| 循环切换货币（¥/$/€） | 弹窗内按 `L`，或 `Ctrl+Shift+L`（可在 keybindings.json 自定义） |
| 在配置中设置语言      | settings.json 的 `deepseekCost.locale`                          |
| 在配置中设置货币      | settings.json 的 `deepseekCost.currency`（EUR 另设 `eurRate`）  |
...
| 配置项     | 默认值    | 说明                                                                           |
| ---------- | --------- | ------------------------------------------------------------------------------ |
| `locale`   | `"zh"`    | UI 语言：`"zh"` 或 `"en"`                                                      |
| `currency` | 随 locale | 显示货币：`"cny"` / `"usd"` / `"eur"`（未设置时按 locale：zh → cny，en → usd） |
| `eurRate`  | `0.92`    | 1 USD 折合 EUR，仅 `"eur"` 时使用。仅供参照 —— 请自行校对你所需的汇率          |
...
- **欧元不是 DeepSeek 官方货币**：当 `currency` 为 `"eur"` 时，官方美元总额按 `deepseekCost.eurRate`（1 USD 折合 EUR）换算，显示带 `≈` 前缀并在面板底部标注所用汇率。默认 `0.92` 仅为参照占位 —— 使用前请自行核实并配置 `eurRate`。
