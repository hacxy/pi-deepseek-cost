...
- **Status bar**: live cumulative cost for the current session (`¥0.019`), refreshed after every turn — cost only, no noise
...
Requires a Pi installation with DeepSeek models configured (`deepseek-v4-flash`, `deepseek-v4-pro`).
...
| Switch language (zh↔en) | `L` inside a panel, or `Ctrl+Shift+L`             |
